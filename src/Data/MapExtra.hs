...
instance (Ord k, DB.Binary k, DB.Binary e) => DB.Binary (Map k e) where
    put = DB.put . toAscList
    get = liftM fromDistinctAscList DB.get

instance (Ord k, SE.Serialize k, SE.Serialize e) => SE.Serialize (Map k e) where
  put = SE.put . toAscList
  get = liftM fromDistinctAscList SE.get

instance (SC.SafeCopy k, SC.SafeCopy e) => SC.SafeCopy (Map k e) where
  getCopy   = SC.contain $ do getter <- SC.getSafeGet
                              return fromDistinctAscList <*> getter

  putCopy m = SC.contain $ do putter <- SC.getSafePut
                              putter $ toAscList m
...
-- Custom additions
lookupLEQ :: Ord k => k -> Map k v -> Maybe (k, v)
lookupLEQ k = go Nothing
  where
    go d Tip = d
    go d (Bin sz ky y l r) = case compare k ky of
      LT -> go d l
      EQ -> return (ky, y)
      GT -> go (return (ky, y)) r
